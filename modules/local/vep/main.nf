...
// Modified for pVACseq tools and option for online run
process ENSEMBLVEP_VEP {
    ...
    def dir_cache = cache ? "--offline --cache --dir_cache \${PWD}/${cache}" : ""
    def reference = fasta ? "--fasta $fasta" : ""
    def compress_cmd = args.contains("--compress_output") ? '' : '--compress_output bgzip'

    def vep_plugins_opt = vep_plugins ? "--dir_plugins ${vep_plugins}" : ""
